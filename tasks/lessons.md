...
26. **느린 생성 작업은 foreground 성공 조건과 분리한다** — 분석 화면에서 모든 cutout을 기다리게 하지 말고, 저장 트랜잭션에는 pending job을 연결한 뒤 내구성 있는 queue가 재시도하도록 한다. 결과 완료 RPC가 연결된 도메인 행까지 같은 트랜잭션에서 갱신해야 한다.

27. **생성 이미지의 그럴듯함은 원본 일치의 증거가 아니다** — bbox가 빈 벽이나 바닥을 가리켜도 카테고리 이름만으로 상품 이미지를 창작할 수 있다. 생성 전 crop grounding과 생성 후 원본 동일성·카테고리 검증을 모두 통과한 결과만 저장하고, 불확실할 때는 가짜 이미지보다 private crop fallback을 선택한다.

28. **상위 AI의 자유 문자열을 하위 AI의 지시문에 재사용하지 않는다** — 이미지에서 유도된 `style_description`도 비신뢰 입력이다. 생성과 검증 프롬프트에는 서버 allowlist 값과 실제 픽셀만 전달해 간접 prompt injection이 여러 품질 게이트를 동시에 오염시키지 못하게 한다.

29. **서로 다른 UI 선택지는 서로 다른 동작 계약을 가져야 한다** — 라벨만 다른 옵션이 같은 API·프롬프트·결과를 사용하면 기대만 분리하고 비용은 중복시킨다. 정식 타입은 하나로 합치되 구형 값은 서버 경계에서 정규화하고, 알 수 없는 값은 비용·쿼터 로직 전에 거절할 것.
